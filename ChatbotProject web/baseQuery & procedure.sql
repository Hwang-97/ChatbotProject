...
as
select a.seq,a.answer,q.question,k.keyword
from tblAnswer a inner join tblquestion q on a.seq = q.answer_seq
                 inner join tblkeyword k on k.answer_seq = a.seq;
...
begin
    Panswer :='No';
    select answer_seq into vseq from tblQuestion where question=Pquestion;
    select answer into Panswer from tblAnswer where seq = vseq;
Exception 
